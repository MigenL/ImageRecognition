...
def edImage(img):
    imgCanny = cv2.Canny(img,100,200) #cv2.Laplacian(img,cv2.CV_64F)
    return imgCanny
...
# ------------------------------------------------------------------------------------------
def fibreVal(imGui,feature,value):
    ...
    imgAcquired = acquireImage(imGui,images)
    ...
    PoimgGui= imGui[imGui.rfind("/")+1:len(imGui)]
    importToDB(images['Original'],featuresTuple,value,PoimgGui)
    print("Succesfully saved image",PoimgGui)
    tkMessageBox.showinfo("Completed","The image ", PoimgGui ," is stored")


    featuresSelected = [i+1 for i,x in enumerate(feature) if x !=0]
